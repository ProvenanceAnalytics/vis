...
```
The server so far will run spade and delete the existing filter. Then it should create a filter for the script uploaded by the user. Then the script is run and spade is stopped. The parser is then run and the data is uploaded to the database so that it can be displayed on the website. (Currently, the command for adding the filter for the users program is not working).

To run this server we must first install the packages required. We do so with: 
```
...
npm install cors
```
Then we boot the server up with `node server2.js` Then the user can click the button to upload their scripts and the commands that are in the server are run.

Currently, the server would only work with Python programs as they do not need to be compiled.
